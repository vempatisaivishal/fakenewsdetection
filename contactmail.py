...
class findMail:
    def __init__(self, link, true, false):
        self.true = true
        self.false = false
        self.link = link
    ...
    def scrape_email_on_web(self):
        if 'www.' in self.domain_name:
            self.domain_name = self.domain_name.replace("www.", '')
    ...
    def run(self):
        ...
        sendMessage(f"hello the {self.false} is a fake news", f"the true news is {self.true}", self.emails, self.social_links).run()

# findMail('https://www.moneycontrol.com/news/india/iit-bombay-ranks-1st-in-india-149th-in-world-in-qs-world-university-rankings-10871391.html', 'hii', 'ignore this').run()